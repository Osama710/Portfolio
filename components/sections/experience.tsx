...
function CareerDetailPanel({
  ...
  return (
    <motion.div
      key={activeIndex}
      initial={reducedMotion ? false : { opacity: 0, x: enterX }}
      ...
      <div className={cn("relative", compact ? "p-4" : "p-5 sm:p-6")}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h3 className="mt-1 font-display text-xl font-bold text-ink sm:text-2xl">{active.role}</h3>
            <p className="mt-0.5 text-sm text-accent-violet sm:text-base">{active.company}</p>
          </div>
          <span
            className={cn(
              "rounded-full px-3 py-1 text-[0.62rem] font-bold uppercase",
...
export function Experience() {
  ...
  return (
    <section id="experience" className="site-section relative overflow-visible">
